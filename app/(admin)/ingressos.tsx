...
export default function Ingressos() {
  ...
  return <Screen variant="admin">
    <Header title="Lotes de ingressos" right={<FloatingActionButton onPress={openNew} accessibilityLabel="Novo lote de ingressos" />} />
    ...
    {loading ? <LoadingState label="Carregando lotes..." /> : null}
    {error ? <ErrorState message={error} onRetry={refetch} /> : null}
    {!loading && !error && !filtered.length ? <EmptyState title="Nenhum lote de ingressos foi gerado para alunos." /> : null}
    ...
    <AppModal visible={open} onClose={() => setOpen(false)} position="center" title="Gerar lote de ingressos" footer={<View style={styles.footer}><View style={styles.footerItem}><Button title="Cancelar" tone="dark" onPress={() => setOpen(false)} /></View><View style={styles.footerItem}><Button title={submitting ? 'Gerando...' : 'Gerar lote'} tone="green" onPress={!submitting && person && eventoId ? generate : undefined} /></View></View>}>
      ...
      <ChoiceGroup options={origins} value={origin} onChange={(value) => setOrigin(value as Origin)} />
      ...
      {origin === 'PAGAMENTO_EXTERNO' ? <ChoiceGroup
        options={(['PIX_EXTERNO', 'DINHEIRO', 'CARTAO_EXTERNO'] as ExternalMethod[]).map((method) => ({ value: method, label: method.replaceAll('_', ' ') }))}
        value={externalMethod}
        onChange={(value) => setExternalMethod(value as ExternalMethod)}
      /> : null}
