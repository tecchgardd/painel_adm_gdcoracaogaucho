...
export async function createEmpresa(input: EmpresaInput) { return unwrapData<Empresa>((await api.post('/admin/empresas', body(input))).data); }
export async function updateEmpresa(id: string, input: EmpresaInput) { return unwrapData<Empresa>((await api.patch(`/admin/empresas/${id}`, body(input))).data); }
